...
import {
  ...
  LayoutGrid, RotateCcw, FileText, Paperclip, LogOut, FileSignature, Building2
} from "lucide-react";
...
export default function App() {
  ...
  const loadFromServer = useCallback(async ({ silent } = {}) => {
    ...
    try {
      ...
      if (data) {
        ...
        await supabase.from("rental_app_state").upsert({ id: ROW_ID, tenants: [], expenses: {}, irpf_reduccion: 0.5, segunda_vivienda: buildEmptySegundaVivienda() });
      }
      setConnError(false);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        const { error } = await supabase.from("rental_app_state").upsert({
          id: ROW_ID, tenants, expenses, irpf_reduccion: irpfReduccion, segunda_vivienda: segunda, updated_at: new Date().toISOString()
        });
        if (error) throw error;
        setConnError(false);
      } catch (e) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tenants, expenses, irpfReduccion, segunda, loaded]);
