...
const ROOM_LABELS = ["A1", "A2", "B1", "B2", "C1", "C2", "D1", "D2", "E1", "F1", "F2", "F3"];
...
function validarContrato(tenant) {
  const obligatorios = [];
...
/* La vivienda de Calle Azorín 6 está registrada catastralmente como dos puertas (bajo
   derecha y bajo izquierda), aunque para el negocio es una única unidad — así que las dos
   referencias van siempre juntas en todos los contratos, sin distinguir qué habitación
   corresponde a cada una. */
const REFERENCIAS_CATASTRALES = "7446704XH5074F0037XH y 7446704XH5074F0038MJ";

async function generarContratoDocx(tenant, numCamas) {
  const { obligatorios } = validarContrato(tenant);
  ...
  const valores = {
    ...
    "@@REF_CATASTRAL@@": REFERENCIAS_CATASTRALES,
...
function TenantFormModal({ tenant, onCancel, onSave, notify, onViewDoc, selectedMonth, onGenerarRecibo, tenants }) {
  ...
  function handleGenerarRecibo(ym) {
    ...
    const etiquetaVivienda = `Habitación ${form.habitacion} — Calle Azorín, 6 Bajo, 30820 Alcantarilla-Murcia`;
    generarReciboPDF(form, DATOS_ARRENDADOR_RECIBO, etiquetaVivienda, ym, numero);
  }
  ...
  async function handleGenerarContrato() {
    const { obligatorios, opcionales } = validarContrato(form);
    if (obligatorios.length) {
    ...
    try {
      ...
      await generarContratoDocx(form, numCamas);
      ...
      <div className="rg-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {tenant?.id && (
          <>
            ...
            {(() => {
              const mesesPagados = Object.keys(form.pagos || {})
                .filter(ym => form.pagos[ym])
                .sort()
                .reverse();
              ...
              return (
                <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
                  {mesesPagados.map(ym => {
                    ...
                    const numeroYaAsignado = form.recibos?.[ym];
                    const numeroMostrable = typeof numeroYaAsignado === "number" ? numeroYaAsignado : null;
                    return (
                      <div key={ym} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 10px", background: "var(--surface-alt)", borderRadius: 8 }}>
                        <span style={{ fontSize: 12 }}>
                          ...
                          {numeroMostrable && <span style={{ color: "var(--text-dim)", marginLeft: 6 }}>· Nº {String(numeroMostrable).padStart(3, "0")}/{anioYm}</span>}
                        </span>
