...
import { CONTRATO_TEMPLATE_B64 } from "./contratoTemplate.js";
import {
  ...
  LayoutGrid, RotateCcw, FileText, Paperclip, LogOut, FileSignature, Building2, Wrench
} from "lucide-react";
...
/* ------------------------------------------------------------------ */
/* Segunda vivienda (Sangonera la Seca) — completamente independiente   */
/* de las habitaciones: ni sus ingresos ni sus gastos se suman entre sí */
/* ------------------------------------------------------------------ */

function buildEmptySegundaVivienda() {
  return {
    datos: { nombre: "", telefono: "", correo: "", fechaInicio: "", fechaFinContrato: "" },
    meses: {}
  };
}
function emptySegundaMes() {
  return {
    renta: "", fechaPagoRenta: "",
    luzImporte: "", fechaPagoLuz: "",
    aguaImporte: "", fechaPagoAgua: "",
    ibiImporte: "", ibiFecha: "",
    reparacionesItems: []
  };
}
function segundaMesData(segunda, ym) {
  return segunda.meses?.[ym] || emptySegundaMes();
}
function segundaIngresosForMonth(segunda, ym) {
  const m = segundaMesData(segunda, ym);
  if (!m.fechaPagoRenta || !m.renta) return 0;
  return Number(m.renta) || 0;
}
function segundaGastosForMonth(segunda, ym) {
  const m = segundaMesData(segunda, ym);
  const ibi = Number(m.ibiImporte) || 0;
  const reparaciones = (m.reparacionesItems || []).reduce((s, it) => s + (Number(it.importe) || 0), 0);
  return ibi + reparaciones;
}
function segundaBeneficioForMonth(segunda, ym) {
  return segundaIngresosForMonth(segunda, ym) - segundaGastosForMonth(segunda, ym);
}
function segundaAcumuladoAnual(segunda, year, fn) {
  let total = 0;
  for (let m = 1; m <= 12; m++) total += fn(segunda, ymKey(year, m));
  return total;
}
/* Acumulado de verdad desde que existe el contrato (2024), no solo del año en curso: suma
   todos los meses que tienen datos guardados, sin importar el año al que pertenezcan. */
function segundaAcumuladoTotal(segunda, fn) {
  const meses = Object.keys(segunda.meses || {});
  let total = 0;
  for (const ym of meses) total += fn(segunda, ym);
  return total;
}
...
// Ubicaciones donde puede darse una incidencia: las habitaciones de siempre, más Sangonera la
// Seca (la segunda vivienda), por si hay algún aviso o avería también ahí.
const INCIDENCIA_UBICACIONES = [...ROOM_LABELS, "Sangonera la Seca"];
...
/* La renta puede cambiar con el tiempo. En vez de un único valor fijo, cada inquilino guarda un
   ...
   entrado en vigor para ese mes — así, cambiar la renta en agosto no reescribe julio ni meses
...
function rentaEnMes(t, ym) {
  ...
  // El mes consultado es anterior a cualquier entrada del historial (por ejemplo, si la
  // primera entrada guardada no cubre desde el inicio real del contrato). En ese caso usamos
  // la entrada más ANTIGUA que tengamos como mejor referencia — nunca la más reciente, que es
  // justo la que causaría el mismo fallo que estamos evitando.
  ...
  tenants
    ...
    .forEach(t => {
      ...
      if (end && end >= monthStartD && end < monthEndD) {
        notes.push(`Fin de contrato: ${fmtDate(effectiveEnd(t))} (${nombre})`);
      }
...
function validarContrato(tenant, refCatastral) {
  const obligatorios = [];
  ...
  if (!refCatastral?.trim()) obligatorios.push("referencia catastral (en Gastos)");

  // Datos de contacto del inquilino: importantes, pero no bloquean la generación — se puede
  // avisar y dejar que decidas generar igualmente, dejando un hueco en blanco.
  const opcionales = [];
...
/* El número que sigue a la letra del código de habitación indica cuántas camas tiene esa
   habitación en total (A1 y A2 son las dos camas de la habitación "A", por ejemplo). Contamos
   cuántos códigos de habitación distintos comparten la misma letra, entre todos los inquilinos
   que ha habido alguna vez — así una cama vacía en este momento sigue contando. */
function letraDeHabitacion(codigo) {
  const m = String(codigo || "").trim().match(/^([A-Za-z]+)/);
  return m ? m[1].toUpperCase() : "";
}
function calcularCamasHabitacion(tenants, habitacionActual) {
  const letra = letraDeHabitacion(habitacionActual);
  if (!letra) return 1;
  const codigosUnicos = new Set(
    (tenants || [])
      .map(t => String(t.habitacion || "").trim())
      .filter(cod => letraDeHabitacion(cod) === letra && cod)
  );
  codigosUnicos.add(String(habitacionActual || "").trim());
  return codigosUnicos.size;
}

async function generarContratoDocx(tenant, refCatastral, numCamas) {
  const { obligatorios } = validarContrato(tenant, refCatastral);
  if (obligatorios.length) {
    throw new Error(`Faltan datos en la ficha para generar el contrato: ${obligatorios.join(", ")}.`);
  }
  ...
  const valores = {
    ...
    "@@FIANZA_MESES@@": meses,
    "@@LUGAR_FIRMA@@": "Alcantarilla",
    "@@FECHA_FIRMA_DIA@@": dia,
    "@@FECHA_FIRMA_MES@@": mes,
    "@@FECHA_FIRMA_ANIO@@": anio,
    "@@REF_CATASTRAL@@": refCatastral.trim(),
    "@@NUM_CAMAS@@": String(numCamas || 1),
    "@@NUM_COMPANEROS@@": String(Math.max((numCamas || 1) - 1, 0)),
  };

  const zip = await JSZip.loadAsync(base64ToUint8Array(CONTRATO_TEMPLATE_B64));
  let xml = await zip.file("word/document.xml").async("text");
  if ((numCamas || 1) > 1) {
    // Habitación compartida: se queda la cláusula, solo se quitan los comentarios marcadores.
    xml = xml.replace("<!--CLAUSULA_COMPARTIDA_INICIO-->", "").replace("<!--CLAUSULA_COMPARTIDA_FIN-->", "");
  } else {
    // Habitación individual: se quita el párrafo entero de la cláusula, marcadores incluidos.
    xml = xml.replace(/<!--CLAUSULA_COMPARTIDA_INICIO-->[\s\S]*?<!--CLAUSULA_COMPARTIDA_FIN-->/, "");
  }
...
/* Recibo de alquiler en PDF — documento simple, no es una factura (un particular no está
   obligado a facturar con IVA por alquilar su vivienda), pero sirve como comprobante de pago
   para el inquilino. La numeración correlativa (si aplica) se calcula fuera de esta función y
   se le pasa ya decidida. */
function generarReciboPDF(tenant, arrendador, etiquetaVivienda, ym, numeroRecibo) {
  const [anio, mesNum] = ym.split("-").map(Number);
  const nombreMes = MESES[mesNum - 1];
  const importe = Number(rentaEnMes(tenant, ym)) || 0;
  const hoy = new Date();
  ...
  const gris = [91, 97, 120];
  const negro = [15, 20, 36];
  const margen = 20;
  let y = 20;

  const logoW = 42, logoH = logoW * (124 / 320);
  doc.addImage(LOGO_APAISADO_B64, "PNG", margen, y, logoW, logoH);
  y += logoH + 14;

  doc.setDrawColor(226, 228, 242);
  ...
  doc.text("RECIBO DE ALQUILER", margen, y);
  if (typeof numeroRecibo === "number") {
    doc.setFontSize(11);
    doc.setTextColor(...azul);
    doc.text(`Nº ${String(numeroRecibo).padStart(3, "0")}/${anio}`, 210 - margen, y, { align: "right" });
  }
  y += 7;
  ...
  fila("Recibí de", `${tenant.nombre || ""} ${tenant.apellidos || ""}`.trim());
  fila("Con documento", `${tenant.tipoDocumento || "DNI"} ${tenant.numeroDocumento || ""}`.trim());
  fila("En concepto de", `Alquiler correspondiente al mes de ${nombreMes} de ${anio}`);
  fila("Vivienda / habitación", etiquetaVivienda || "—");

  doc.setFillColor(245, 246, 251);
  doc.roundedRect(margen, y, 210 - margen * 2, 18, 2, 2, "F");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
  doc.setTextColor(...gris);
  doc.text("IMPORTE", margen + 6, y + 7);
  doc.setFontSize(15);
  doc.setTextColor(...azul);
  doc.text(`${importe.toFixed(2).replace(".", ",")} €`, margen + 6, y + 14);
  y += 28;
  ...
  fila("Propietario/a", arrendador.nombre || "—");
  fila("NIF / DNI", arrendador.documento || "—");
  fila("Domicilio", arrendador.domicilio || "—");

  // Sello de certificación — es lo que da validez al recibo. Un recibo sin este bloque no ha
  // sido emitido a través de Susalquia por el propietario, y no debe considerarse válido.
  y += 2;
  const textoCert = `Este recibo ha sido generado y emitido por ${arrendador.nombre || "el propietario/a"} a través de Susalquia (susalquia.com), plataforma de gestión de alquileres. Un recibo de alquiler que no incluya esta certificación no ha sido emitido por esta vía y no debe considerarse válido.`;
  const lineasCert = doc.splitTextToSize(textoCert, 210 - margen * 2 - 12);
  const altoCert = 12 + lineasCert.length * 4.2;
  doc.setDrawColor(...azul);
  doc.setLineWidth(0.5);
  doc.setFillColor(238, 240, 255);
  doc.roundedRect(margen, y, 210 - margen * 2, altoCert, 2, 2, "FD");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8.5);
  doc.setTextColor(...azul);
  doc.setFillColor(...azul);
  doc.circle(margen + 8, y + 5.3, 1.6, "F");
  doc.setDrawColor(255, 255, 255);
  doc.setLineWidth(0.5);
  doc.line(margen + 7.1, y + 5.3, margen + 7.7, y + 6);
  doc.line(margen + 7.7, y + 6, margen + 9, y + 4.5);
  doc.text("RECIBO CERTIFICADO POR SUSALQUIA", margen + 12, y + 7);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(...negro);
  doc.text(lineasCert, margen + 6, y + 12.5);
  y += altoCert + 10;

  doc.setFont("helvetica", "italic");
  doc.setFontSize(8.5);
  doc.setTextColor(...gris);
  doc.text("Este recibo no constituye factura y no está sujeto a IVA.", margen, y);
  doc.text("Generado con Susalquia — susalquia.com", margen, y + 5);

  const nombreArchivo = `recibo_${(tenant.nombre || "inquilino").replace(/\s+/g, "_")}_${ym}.pdf`;
...
const LOGO_APAISADO_B64 = "";

/* Datos de la propietaria para los recibos — igual que "Alcantarilla" en el contrato, van
   escritos directamente aquí porque esta app no tiene una pantalla de "Mi cuenta" donde
   guardarlos. Si cambian, se edita directamente esta constante. */
const DATOS_ARRENDADOR_RECIBO = {
  nombre: "Susana Puche Ruiz",
  documento: "52827604P",
  domicilio: "Calle Parque 14, 30833, Sangonera la Verde - Murcia",
};
...
const GlobalStyles = () => (
  <style>{`
    ...
    .rg-brand-stack { flex-direction: column; align-items: flex-start; }
    .rg-brand-logo { height: 34px; width: auto; display: block; }
    .rg-brand-mark {
    ...
    .rg-date-clear-wrap { position: relative; }
    .rg-date-clear-wrap .rg-input { padding-right: 34px; }
    .rg-date-clear-wrap input[type="date"]::-webkit-clear-button { display: none; }
    .rg-date-clear-btn {
      position: absolute; right: 30px; top: 50%; transform: translateY(-50%);
      width: 20px; height: 20px; display: flex; align-items: center; justify-content: center;
      background: var(--surface-alt); color: var(--text-dim); border: none; border-radius: 50%;
      cursor: pointer; padding: 0;
    }
    .rg-date-clear-btn:hover { background: var(--danger-dim); color: var(--danger); }
    .rg-pago-status { font-size: 11px; margin-top: 5px; }
    .rg-pago-status.is-paid { color: var(--ok); }
    .rg-pago-status.is-pending { color: var(--text-dim); }
  ...
  return (
    <div className="rg-root" style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", padding: 20 }}>
      ...
      <form className="rg-card rg-login-card" onSubmit={handleSubmit}>
        <div className="rg-brand rg-brand-stack" style={{ marginBottom: 22, padding: 0 }}>
          <img src="data:image/png;base64," alt="Susalquia" className="rg-brand-logo" />
          <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 6 }}>La forma fácil de gestionar tus alquileres</div>
        </div>
...
export default function App() {
  ...
  const [segunda, setSegunda] = useState(() => buildEmptySegundaVivienda());
  ...
  const loadFromServer = useCallback(async ({ silent } = {}) => {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.segunda_vivienda && typeof data.segunda_vivienda === "object") {
          setSegunda({ ...buildEmptySegundaVivienda(), ...data.segunda_vivienda });
        }
        setIncidencias(Array.isArray(data.incidencias) ? data.incidencias : []);
      } else {
        await supabase.from("rental_app_state").upsert({ id: ROW_ID, tenants: [], expenses: {}, irpf_reduccion: 0.5, segunda_vivienda: buildEmptySegundaVivienda(), incidencias: [] });
      }
      setConnError(false);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        const { error } = await supabase.from("rental_app_state").upsert({
          id: ROW_ID, tenants, expenses, irpf_reduccion: irpfReduccion, segunda_vivienda: segunda, incidencias, updated_at: new Date().toISOString()
        });
        if (error) throw error;
        setConnError(false);
      } catch (e) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tenants, expenses, irpfReduccion, segunda, incidencias, loaded]);
  ...
  function incomeEmpadronadosForMonth(ym) {
    return tenants.reduce((sum, t) => sum + (t.empadronado && t.pagos && t.pagos[ym] ? rentaEnMes(t, ym) : 0), 0);
  }
  ...
  function getRefCatastral() {
    return expenses._settings?.refCatastral || "";
  }
  ...
  /* La numeración correlativa empieza en 2027 — antes de esa fecha, los recibos no llevan
     número. Pero en los dos casos, hace falta generar el recibo al menos una vez para que el
     inquilino pueda verlo en su Portal — así queda claro que un recibo solo existe porque tú lo
     has emitido, evitando cualquier disputa sobre si un pago está confirmado o no. Cada
     inquilino guarda, dentro de su propia ficha, qué recibos ya se han emitido (con su número,
     si le corresponde) — y la cuenta de "cuál es el siguiente número" se guarda a nivel de
     cuenta, por año. Esto se actualiza aquí mismo, directamente sobre los datos guardados — no
     depende de pulsar "Guardar" en la ficha. */
  const ANIO_INICIO_NUMERACION_RECIBOS = 2027;
  function obtenerOAsignarNumeroRecibo(tenantId, ym) {
    const anio = ym.slice(0, 4);
    const requiereNumero = Number(anio) >= ANIO_INICIO_NUMERACION_RECIBOS;

    const tenantActual = tenants.find(t => t.id === tenantId);
    if (tenantActual?.recibos?.[ym]) return tenantActual.recibos[ym];

    let valorAsignado = true;
    if (requiereNumero) {
      const contadores = expenses._settings?.contadoresRecibos || {};
      const siguiente = (contadores[anio] || 0) + 1;
      updateGestionConfig({ contadoresRecibos: { ...contadores, [anio]: siguiente } });
      valorAsignado = siguiente;
    }

    setTenants(prev => prev.map(t => (t.id === tenantId ? { ...t, recibos: { ...(t.recibos || {}), [ym]: valorAsignado } } : t)));
  ...
  const occupancyStats = useMemo(() => {
    ...
    const capacityRoomMonths = TOTAL_HABITACIONES * activeMonths.length;
    return {
  ...
  const alerts = useMemo(() => {
    ...
    /* Sangonera la Seca: renta, luz y agua las paga la inquilina en mano cada mes.
       A partir de julio de 2026, si ya ha pasado el día 5 del mes y no se ha
       marcado como cobrado, avisamos — sin esto es fácil que se pase por alto,
       ya que no hay ningún recibo bancario que lo recuerde. */
    const segundaPagosPendientes = [];
    const gestionInicioD = toDate(GESTION_INICIO);
    const currentYmD = toDate(currentYm + "-01");
    const diaHoy = now.getDate();
    if (gestionInicioD && currentYmD && currentYmD >= gestionInicioD && diaHoy >= 5) {
      const m = segundaMesData(segunda, currentYm);
      if (!m.fechaPagoRenta) segundaPagosPendientes.push({ concepto: "Renta", importe: m.renta });
      if (!m.fechaPagoLuz) segundaPagosPendientes.push({ concepto: "Luz", importe: m.luzImporte });
      if (!m.fechaPagoAgua) segundaPagosPendientes.push({ concepto: "Agua", importe: m.aguaImporte });
    }

    return { impagos, finContrato, fianzaNoCobrada, fianzaNoDevuelta, segundaPagosPendientes };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tenants, segunda]);

  const totalAlerts = alerts.impagos.length + alerts.finContrato.length + alerts.fianzaNoCobrada.length + alerts.fianzaNoDevuelta.length + alerts.segundaPagosPendientes.length;
  ...
  function updateSegundaDatos(patch) {
    setSegunda(prev => ({ ...prev, datos: { ...prev.datos, ...patch } }));
  }
  function updateSegundaMes(ym, patch) {
    setSegunda(prev => ({
      ...prev,
      meses: { ...prev.meses, [ym]: { ...(prev.meses[ym] || emptySegundaMes()), ...patch } }
    }));
  }
  function updateSegundaItems(ym, field, items) {
    setSegunda(prev => ({
      ...prev,
      meses: { ...prev.meses, [ym]: { ...(prev.meses[ym] || emptySegundaMes()), [field]: items } }
    }));
  }
  ...
  async function uploadPresupuestoIncidencia(id, file) {
    try {
      const path = `incidencias/${id}/presupuesto/${Date.now()}_${sanitizeFileName(file.name)}`;
    ...
    <div className="rg-root">
      ...
      <div className="rg-shell">
        ...
        <div className="rg-sidebar">
          <div className="rg-brand rg-brand-stack">
            <img src="data:image/png;base64," alt="Susalquia" className="rg-brand-logo" />
            <div style={{ fontSize: 9.5, color: "var(--text-dim)", letterSpacing: ".02em", lineHeight: 1.3, marginTop: 6 }}>La forma fácil de gestionar tus alquileres</div>
          </div>
          ...
          <button className={`rg-nav-item ${view === "segunda" ? "active" : ""}`} onClick={() => setView("segunda")}>
            <Building2 size={16} /> Sangonera la Seca
          </button>
          ...
          <div className="rg-sidebar-footer">
            ...
            <button className="rg-icon-btn rg-logout-btn" onClick={handleSignOut} title="Cerrar sesión">
              <LogOut size={14} /> <span style={{ fontSize: 11 }}>Cerrar sesión</span>
            </button>
          </div>
        </div>
        ...
        <div className="rg-main">
          ...
          {view === "gastos" && (
            <GastosView
              ...
              refCatastral={getRefCatastral()} updateRefCatastral={(v) => updateGestionConfig({ refCatastral: v })}
            />
          )}

          {view === "incidencias" && (
            <IncidenciasView
              incidencias={incidencias} ubicaciones={INCIDENCIA_UBICACIONES}
          ...
          {view === "historico" && (
            <HistoricoView tenants={tenants} onEdit={openEditTenant} onExport={handleExportHistorico} onReactivate={reactivateTenant} onDelete={setConfirmDelete} />
          )}

          {view === "segunda" && (
            <SegundaViviendaView
              segunda={segunda} selectedMonth={selectedMonth} monthLabel={monthLabel} shiftMonth={shiftMonth} selYear={selYear}
              onUpdateDatos={updateSegundaDatos} onUpdateMes={updateSegundaMes} onUpdateItems={updateSegundaItems}
            />
          )}
        </div>
      </div>

      <button className="rg-mobile-logout" onClick={handleSignOut}>
      ...
      {showForm && editingTenant && (
        <TenantFormModal
          ...
          onGenerarRecibo={obtenerOAsignarNumeroRecibo}
          tenants={tenants}
        />
      )}
    ...
    <>
      ...
      <div className="rg-card" style={{ padding: 20 }}>
        <div className="rg-topbar" style={{ marginBottom: 14 }}>
          ...
          <YearPicker year={calYear} onChange={setCalYear} />
        </div>
        <div className="rg-table-wrap">
              ...
              {rooms.map(label => (
                <tr key={label}>
                  ...
                  {MESES_CORTOS.map((_, idx) => {
                    const { percent, tooltip } = roomMonthOccupancy(tenants, label, calYear, idx + 1);
                    if (percent === null) {
                      return (
                      ...
                      <td key={idx} title={tooltip}>
                        <div className="rg-occ-track">
                          {(() => {
                            ...
                            return (
                              <>
                                {dispOcupado > 0 && <div className="rg-occ-fill rg-occ-fill-ocupado" style={{ width: `${dispOcupado}%` }} />}
                                {dispLibre > 0 && <div className="rg-occ-fill rg-occ-fill-libre" style={{ width: `${dispLibre}%` }} />}
                              </>
                            );
                          })()}
                        </div>
...
function GastosView({ selectedMonth, monthLabel, shiftMonth, expenses, updateExpenseField, updateExpenseItems, incomeForMonth, fixedExpensesForMonth, managementFeeForMonth, totalExpensesForMonth, netProfitForMonth, yearData, selYear, onUploadFactura, onDeleteFactura, onViewDoc, updateGestionConfig, refCatastral, updateRefCatastral }) {
  ...
  return (
    <>
      ...
      <div className="rg-card" style={{ padding: 20, marginBottom: 18 }}>
        <div className="rg-section-title">Datos de la vivienda para el contrato</div>
        <div className="rg-field" style={{ maxWidth: 340 }}>
          <label className="rg-label">Referencia catastral</label>
          <input
            className="rg-input" value={refCatastral}
            onChange={(e) => updateRefCatastral(e.target.value.toUpperCase())}
          ...
          <div style={{ fontSize: 11, color: "var(--text-dim)", marginTop: 4 }}>
            ...
            Catastro (sedecatastro.gob.es). Se usa al generar el contrato.
          </div>
        </div>
      </div>
...
function AlertasView({ alerts, onSelectTenant }) {
  return (
    <>
      ...
      <AlertGroup
        title="Sangonera la Seca — pagos pendientes (a partir del día 5)" icon={<Building2 size={17} />} color="var(--danger)"
        items={alerts.segundaPagosPendientes} emptyText="Sin pagos pendientes de marcar este mes en Sangonera la Seca."
        renderItem={(item, i) => (
          <div className="rg-alert-item" key={item.concepto}>
            <div>
              <div className="rg-alert-item-name">{item.concepto} sin cobrar</div>
              <div className="rg-alert-item-sub">Sangonera la Seca · Ya ha pasado el día 5 del mes</div>
            </div>
            <span className="rg-badge rg-badge-danger">{item.importe ? fmtMoney(item.importe) : "Sin importe"}</span>
          </div>
        )}
      />
    </>
...
function IncidenciasView({ incidencias, ubicaciones, onAdd, onUpdate, onDelete, onUploadPresupuesto, onEliminarPresupuesto, onViewDoc }) {
  ...
  return (
    <>
      ...
      {visibles.length === 0 ? (
        <div className="rg-card" style={{ padding: 24, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 12.5, color: "var(--text-dim)" }}>
            Registra aquí averías, reparaciones pendientes o cualquier aviso de una habitación o de Sangonera la Seca.
          </div>
        </div>
      ) : (
      ...
      {showForm && (
        <IncidenciaFormModal ubicaciones={ubicaciones} onSave={onAdd} onClose={() => setShowForm(false)} />
      )}
...
function IncidenciaCard({ incidencia, onUpdate, onDelete, onUploadPresupuesto, onEliminarPresupuesto, onViewDoc }) {
  const fileInputRef = useRef(null);
    ...
    <div className="rg-card" style={{ padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 8 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--text-dim)" }}>
            {incidencia.habitacion && <>{incidencia.habitacion} · </>}
...
function IncidenciaFormModal({ ubicaciones, onSave, onClose }) {
  ...
  return (
    <div className="rg-modal-overlay" onClick={onClose}>
      <div className="rg-modal" style={{ maxWidth: 520 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rg-form-grid">
          <div className="rg-field">
            <label className="rg-label">Habitación o zona común (opcional)</label>
            <select className="rg-select" value={habitacion} onChange={(e) => setHabitacion(e.target.value)}>
              <option value="">Sin especificar</option>
              {ubicaciones.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
          </div>
          <div className="rg-field">
...
function HistoricoView({ tenants, onEdit, onExport, onReactivate, onDelete }) {
...
/* ------------------------------------------------------------------ */
/* Segunda vivienda (Sangonera la Seca)                                 */
/* ------------------------------------------------------------------ */

/* Campo de fecha con botón de borrado explícito: evita el problema de los inputs
   de fecha nativos en móvil, que a veces muestran hoy en gris y no dejan vaciarse. */
function DateClearField({ label, value, onChange, showStatus, paidLabel = "Pagado" }) {
  return (
    <div className="rg-field">
      <label className="rg-label">{label}</label>
      <div className="rg-date-clear-wrap">
        <input
          className="rg-input" type="date" value={value || ""}
          onChange={(e) => onChange(e.target.value)}
        />
        {!!value && (
          <button
            type="button" className="rg-date-clear-btn"
            onClick={() => onChange("")} title="Borrar fecha" aria-label="Borrar fecha"
          >
            <X size={13} />
          </button>
        )}
      ...
      {showStatus && (
        <div className={`rg-pago-status ${value ? "is-paid" : "is-pending"}`}>
          {value ? `${paidLabel} el ${fmtDate(value)}` : "Sin marcar todavía"}
        </div>
      )}
    </div>
  );
...
function SegundaViviendaView({ segunda, selectedMonth, monthLabel, shiftMonth, selYear, onUpdateDatos, onUpdateMes, onUpdateItems }) {
  const datos = segunda.datos || {};
  const mes = segundaMesData(segunda, selectedMonth);

  const ingresosMes = segundaIngresosForMonth(segunda, selectedMonth);
  const gastosMes = segundaGastosForMonth(segunda, selectedMonth);
  const beneficioMes = segundaBeneficioForMonth(segunda, selectedMonth);
  const ingresosAcum = segundaAcumuladoAnual(segunda, selYear, segundaIngresosForMonth);
  const gastosAcum = segundaAcumuladoAnual(segunda, selYear, segundaGastosForMonth);
  const beneficioAcum = ingresosAcum - gastosAcum;

  // Desde el inicio real del contrato (2024), no solo del año en curso.
  const ingresosTotal = segundaAcumuladoTotal(segunda, segundaIngresosForMonth);
  const gastosTotal = segundaAcumuladoTotal(segunda, segundaGastosForMonth);
  const beneficioTotal = ingresosTotal - gastosTotal;

  function addItem(field, defaultConcepto) {
    const current = mes[field] || [];
    onUpdateItems(selectedMonth, field, [...current, { id: uid(), concepto: defaultConcepto, importe: "" }]);
  }
  function updateItem(field, id, patch) {
    const current = mes[field] || [];
    onUpdateItems(selectedMonth, field, current.map(it => (it.id === id ? { ...it, ...patch } : it)));
  }
  function removeItem(field, id) {
    const current = mes[field] || [];
    onUpdateItems(selectedMonth, field, current.filter(it => it.id !== id));
  }
  ...
  return (
    <>
      <div className="rg-topbar">
        <div>
          <h1 className="rg-h1">Sangonera la Seca</h1>
          <div className="rg-sub">Vivienda de larga estancia (contrato de 5 años) · independiente de las habitaciones</div>
        </div>
        <MonthPicker monthLabel={monthLabel} shiftMonth={shiftMonth} />
      </div>

      <div className="rg-card" style={{ padding: 20, marginBottom: 18 }}>
        <div className="rg-section-title">Datos del inquilino y del contrato</div>
        <div className="rg-form-grid">
          <div className="rg-field">
            <label className="rg-label">Nombre y apellidos</label>
            <input className="rg-input" value={datos.nombre || ""} onChange={(e) => onUpdateDatos({ nombre: e.target.value })} placeholder="Nombre completo" />
          </div>
          <div className="rg-field">
            <label className="rg-label">Teléfono</label>
            <input className="rg-input" value={datos.telefono || ""} onChange={(e) => onUpdateDatos({ telefono: e.target.value })} placeholder="600 000 000" />
          </div>
          <div className="rg-field">
            <label className="rg-label">Correo electrónico</label>
            <input className="rg-input" type="email" value={datos.correo || ""} onChange={(e) => onUpdateDatos({ correo: e.target.value })} placeholder="correo@ejemplo.com" />
          </div>
          <div />
          <DateClearField label="Fecha de inicio de contrato" value={datos.fechaInicio} onChange={(v) => onUpdateDatos({ fechaInicio: v })} />
          <DateClearField label="Fecha de fin de contrato" value={datos.fechaFinContrato} onChange={(v) => onUpdateDatos({ fechaFinContrato: v })} />
        </div>
      </div>

      <div className="rg-card" style={{ padding: 20, marginBottom: 18 }}>
        <div className="rg-section-title">Registro de pagos de {monthLabel}</div>
        <div style={{ fontSize: 11.5, color: "var(--text-dim)", marginTop: -6, marginBottom: 12 }}>
          La renta cambia con el tiempo según lo pactado — introdúcela cada mes, no se rellena sola.
        </div>
        <div className="rg-form-grid">
          <div className="rg-field">
            <label className="rg-label">Renta de este mes (€)</label>
            <input className="rg-input" type="number" min="0" step="0.01" value={mes.renta ?? ""} onChange={(e) => onUpdateMes(selectedMonth, { renta: e.target.value === "" ? "" : Number(e.target.value) })} placeholder="0" />
          </div>
          <DateClearField label="Fecha de pago de la renta" value={mes.fechaPagoRenta} onChange={(v) => onUpdateMes(selectedMonth, { fechaPagoRenta: v })} showStatus paidLabel="Cobrada" />
          <div className="rg-field">
            <label className="rg-label">Luz — importe (€)</label>
            <input className="rg-input" type="number" min="0" step="0.01" value={mes.luzImporte ?? ""} onChange={(e) => onUpdateMes(selectedMonth, { luzImporte: e.target.value === "" ? "" : Number(e.target.value) })} placeholder="0" />
          </div>
          <DateClearField label="Luz — fecha de pago" value={mes.fechaPagoLuz} onChange={(v) => onUpdateMes(selectedMonth, { fechaPagoLuz: v })} showStatus />
          <div className="rg-field">
            <label className="rg-label">Agua — importe (€)</label>
            <input className="rg-input" type="number" min="0" step="0.01" value={mes.aguaImporte ?? ""} onChange={(e) => onUpdateMes(selectedMonth, { aguaImporte: e.target.value === "" ? "" : Number(e.target.value) })} placeholder="0" />
          </div>
          <DateClearField label="Agua — fecha de pago" value={mes.fechaPagoAgua} onChange={(v) => onUpdateMes(selectedMonth, { fechaPagoAgua: v })} showStatus />
        </div>
        <div style={{ fontSize: 11.5, color: "var(--text-dim)", marginTop: -6, marginBottom: 4 }}>
          La luz y el agua corren a cargo de la inquilina — se registran aquí solo para llevar el control de sus importes y fechas de pago, no cuentan como gasto de la propiedad.
        </div>
      </div>

      <div className="rg-card" style={{ padding: 20, marginBottom: 18 }}>
        <div className="rg-section-title">Gastos a cargo de la propiedad — {monthLabel}</div>
        <div className="rg-form-grid">
          <div className="rg-field">
            <label className="rg-label">IBI — importe (€)</label>
            <input className="rg-input" type="number" min="0" step="0.01" value={mes.ibiImporte ?? ""} onChange={(e) => onUpdateMes(selectedMonth, { ibiImporte: e.target.value === "" ? "" : Number(e.target.value) })} placeholder="0" />
          </div>
          <DateClearField label="IBI — fecha de pago" value={mes.ibiFecha} onChange={(v) => onUpdateMes(selectedMonth, { ibiFecha: v })} showStatus paidLabel="Pagado" />
        </div>

        <ItemsBlock
          selectedMonth={selectedMonth} field="reparacionesItems" label="Conceptos de reparación"
          conceptos={REPARACIONES_CONCEPTOS} addLabel="Añadir reparación"
          items={mes.reparacionesItems || []}
          onAddItem={addItem} onUpdateItem={updateItem} onRemoveItem={removeItem}
        />
      </div>

      <div className="rg-card" style={{ padding: 20 }}>
        <div className="rg-section-title">Resumen económico de esta vivienda</div>
        <div className="rg-grid-cards cols-3">
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Ingresos del mes</div>
            <div className="rg-stat-value" style={{ color: "var(--ok)" }}>{fmtMoney(ingresosMes)}</div>
          </div>
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Gastos del mes</div>
            <div className="rg-stat-value" style={{ color: "var(--danger)" }}>{fmtMoney(gastosMes)}</div>
          </div>
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Beneficio del mes</div>
            <div className="rg-stat-value" style={{ color: beneficioMes >= 0 ? "var(--ok)" : "var(--danger)" }}>{fmtMoney(beneficioMes)}</div>
          </div>
        </div>
        <div className="rg-grid-cards cols-3" style={{ marginBottom: 0 }}>
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Ingresos acumulados {selYear}</div>
            <div className="rg-stat-value">{fmtMoney(ingresosAcum)}</div>
          </div>
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Gastos acumulados {selYear}</div>
            <div className="rg-stat-value">{fmtMoney(gastosAcum)}</div>
          </div>
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Beneficio acumulado {selYear}</div>
            <div className="rg-stat-value" style={{ color: beneficioAcum >= 0 ? "var(--ok)" : "var(--danger)" }}>{fmtMoney(beneficioAcum)}</div>
          </div>
        </div>

        <div className="rg-section-title" style={{ marginTop: 24 }}>Desde el inicio del contrato{datos.fechaInicio ? ` (${fmtDate(datos.fechaInicio)})` : ""}</div>
        <div className="rg-grid-cards cols-3" style={{ marginBottom: 0 }}>
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Ingresos totales acumulados</div>
            <div className="rg-stat-value">{fmtMoney(ingresosTotal)}</div>
          </div>
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Gastos totales (IBI + reparaciones)</div>
            <div className="rg-stat-value">{fmtMoney(gastosTotal)}</div>
          </div>
          <div className="rg-card rg-stat">
            <div className="rg-stat-label">Diferencia total acumulada</div>
            <div className="rg-stat-value" style={{ color: beneficioTotal >= 0 ? "var(--ok)" : "var(--danger)" }}>{fmtMoney(beneficioTotal)}</div>
          </div>
        </div>
        <div style={{ fontSize: 11.5, color: "var(--text-dim)", marginTop: 8 }}>
          Los suministros (luz y agua) no se incluyen aquí porque los paga la inquilina directamente.
        </div>
      </div>
    </>
  );
...
function TenantFormModal({ tenant, onCancel, onSave, notify, onViewDoc, selectedMonth, refCatastral, onGenerarRecibo, tenants }) {
  ...
  function handleGenerarRecibo(ym) {
    const numero = onGenerarRecibo ? onGenerarRecibo(tenant.id, ym) : null;
    generarReciboPDF(tenant, DATOS_ARRENDADOR_RECIBO, tenant.habitacion, ym, numero);
  }
  ...
  async function handleGenerarContrato() {
    const { obligatorios, opcionales } = validarContrato(form, refCatastral);
    if (obligatorios.length) {
    ...
    try {
      const numCamas = calcularCamasHabitacion(tenants, form.habitacion);
      await generarContratoDocx(form, refCatastral, numCamas);
      ...
      <div className="rg-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rg-doc-block">
          <div className="rg-doc-block-header">
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button type="button" className="rg-btn rg-btn-ghost" onClick={handleGenerarContrato} disabled={generandoContrato}>
                <FileSignature size={13} /> {generandoContrato ? "Generando…" : "Generar contrato"}
              </button>
        ...
        {tenant?.id && (
          <>
            <div className="rg-section-title">Recibos</div>
            {(() => {
              const mesesPagados = Object.keys(tenant.pagos || {})
                .filter(ym => tenant.pagos[ym])
                .sort()
                .reverse();
              if (mesesPagados.length === 0) {
                return <div style={{ fontSize: 11.5, color: "var(--text-dim)", marginBottom: 14 }}>Todavía no hay ningún mes marcado como pagado y guardado.</div>;
              }
              return (
                <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
                  {mesesPagados.map(ym => {
                    const [anioYm, mesYm] = ym.split("-").map(Number);
                    const numeroYaAsignado = tenant.recibos?.[ym];
                    return (
                      <div key={ym} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 10px", background: "var(--surface-alt)", borderRadius: 8 }}>
                        <span style={{ fontSize: 12 }}>
                          {MESES[mesYm - 1]} {anioYm}
                          {numeroYaAsignado && <span style={{ color: "var(--text-dim)", marginLeft: 6 }}>· Nº {String(numeroYaAsignado).padStart(3, "0")}/{anioYm}</span>}
                        </span>
                        <button type="button" className="rg-btn rg-btn-ghost" style={{ fontSize: 11, padding: "4px 10px" }} onClick={() => handleGenerarRecibo(ym)}>
                          Descargar recibo
                        </button>
                      </div>
                    );
