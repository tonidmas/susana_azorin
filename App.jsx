...
import {
  ...
  LayoutGrid, RotateCcw, FileText, Paperclip, LogOut, FileSignature
} from "lucide-react";
...
const GlobalStyles = () => (
  <style>{`
    ...
    .rg-occ-track { width: 100%; min-width: 34px; height: 20px; border-radius: 5px; background: var(--border); overflow: hidden; margin: 0 auto; }
    .rg-occ-fill { height: 100%; background: var(--ok); border-radius: 5px; transition: width .2s ease; }
  ...
  return (
    <>
      ...
      <div className="rg-card" style={{ padding: 20 }}>
                    ...
                    return (
                      <td key={idx} title={tooltip}>
                        <div className="rg-occ-track">
                          {percent > 0 && (
                            <div className="rg-occ-fill" style={{ width: `${Math.max(percent, 6)}%` }} />
                          )}
                        </div>
                      </td>
                    );
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, fontSize: 11.5, color: "var(--text-dim)" }}>
          <div className="rg-occ-track" style={{ width: 30, display: "inline-block" }}><div className="rg-occ-fill" style={{ width: "100%" }} /></div>
          Ocupada · el color se llena en proporción a los días ocupados del mes · pasa el cursor sobre un mes para ver la fecha exacta
        </div>
      </div>
    </>
  );
