...
function rentaEnMes(t, ym) {
  ...
  // Inquilinos ya existentes de antes de este cambio, sin historial todavía: usamos la renta
  // actual como mejor aproximación disponible para meses pasados.
  return Number(t.renta) || 0;
}
function fmtMoney(n) {
  const v = Number(n) || 0;
...
async function generarContratoDocx(tenant) {
  const faltantes = [];
  if (!tenant.nombre?.trim()) faltantes.push("nombre");
  if (!tenant.apellidos?.trim()) faltantes.push("apellidos");
  if (!tenant.numeroDocumento?.trim()) faltantes.push("documento de identidad");
  if (!tenant.nacionalidad?.trim()) faltantes.push("nacionalidad");
  if (!tenant.telefono?.trim()) faltantes.push("teléfono");
  if (!tenant.correo?.trim()) faltantes.push("correo electrónico");
  if (!tenant.habitacion?.trim()) faltantes.push("habitación");
  if (!tenant.fechaInicio) faltantes.push("fecha de inicio de contrato");
  if (!tenant.fechaFin) faltantes.push("fecha de fin de contrato");
  if (!tenant.renta) faltantes.push("renta mensual");
  if (!tenant.fianzaImporte) faltantes.push("importe de la fianza");
  if (faltantes.length) {
    throw new Error(`Faltan datos en la ficha para generar el contrato: ${faltantes.join(", ")}.`);
  }
  ...
  const valores = {
    ...
    "@@NACIONALIDAD@@": tenant.nacionalidad.trim(),
    "@@DOCUMENTO_NUM@@": tenant.numeroDocumento.trim(),
    "@@TELEFONO@@": tenant.telefono.trim(),
    "@@CORREO@@": tenant.correo.trim(),
...
export default function App() {
  ...
  function saveTenant(t) {
    ...
    setTenants(prev => {
      ...
      if (!existing) {
        // Inquilino nuevo: el historial arranca con la renta pactada, vigente desde el mes de
        // inicio del contrato (no desde el mes que se esté viendo ahora mismo).
        const mesInicio = t.fechaInicio ? t.fechaInicio.slice(0, 7) : selectedMonth;
        tenantToSave = { ...t, historialRenta: [{ desde: mesInicio, importe: Number(t.renta) || 0 }] };
        return [...prev, tenantToSave];
      }

      const rentaAnterior = rentaEnMes(existing, selectedMonth);
      const rentaNueva = Number(t.renta) || 0;
      let historial = Array.isArray(existing.historialRenta) ? existing.historialRenta : [];

      // Si es la primera vez que se toca la renta de este inquilino desde que existe el
      // historial (inquilinos ya dados de alta antes de esta función), rellenamos primero la
      // renta que tenía hasta ahora, desde el inicio de su contrato — si no, los meses
      // anteriores al cambio se quedarían sin ninguna referencia y acabarían mostrando por
      // error la renta nueva, en vez de la que tenían de verdad.
      if (historial.length === 0) {
        const mesInicioExistente = existing.fechaInicio ? existing.fechaInicio.slice(0, 7) : selectedMonth;
        historial = [{ desde: mesInicioExistente, importe: rentaAnterior }];
      }

      if (rentaNueva !== rentaAnterior) {
        // La renta ha cambiado: se registra como vigente a partir del mes que se está viendo
        // ahora mismo, sin tocar ninguna entrada anterior — así los meses ya pasados conservan
        // la renta que tenían en su momento.
        historial = [
          ...historial.filter(e => e.desde !== selectedMonth),
          { desde: selectedMonth, importe: rentaNueva },
        ];
      }

      tenantToSave = { ...t, historialRenta: historial };
      return prev.map(p => (p.id === t.id ? tenantToSave : p));
    });
    setShowForm(false);
...
function TenantFormModal({ tenant, onCancel, onSave, notify, onViewDoc }) {
  ...
  async function handleGenerarContrato() {
    ...
    try {
      await generarContratoDocx(form);
