...
function generarReciboPDF(tenant, arrendador, etiquetaVivienda, ym, numeroRecibo) {
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...gris);
  doc.text("Este recibo no constituye factura y no está sujeto a IVA.", margen, 275);
  doc.text("Generado con Susalquia — susalquia.com", margen, 280);
...
export default function App() {
  ...
  /* La numeración correlativa de recibos empieza en 2027 — los recibos de meses anteriores no
     llevan número. Cada inquilino guarda, dentro de su propia ficha, qué número le tocó a cada
     mes ya emitido (para no asignarle uno distinto si se vuelve a descargar) — y la cuenta de
     "cuál es el siguiente número" se guarda a nivel de cuenta, por año. Esto se actualiza aquí
     mismo, directamente sobre los datos guardados — no depende de pulsar "Guardar" en la ficha. */
  const ANIO_INICIO_NUMERACION_RECIBOS = 2027;
  function obtenerOAsignarNumeroRecibo(tenantId, ym) {
    const anio = ym.slice(0, 4);
    if (Number(anio) < ANIO_INICIO_NUMERACION_RECIBOS) return null;
    ...
    setTenants(prev => prev.map(t => (t.id === tenantId ? { ...t, recibos: { ...(t.recibos || {}), [ym]: siguiente } } : t)));
