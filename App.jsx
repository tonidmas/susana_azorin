...
function matchesRoom(habitacion, roomNumber) {
  const h = String(habitacion || "").trim();
  if (!h) return false;
  if (h === String(roomNumber)) return true;
  const n = Number(h);
  return !Number.isNaN(n) && n === roomNumber;
}
...
function HabitacionesView({ tenants, selectedMonth, onEdit, onFree, onAddForRoom }) {
  const rooms = Array.from({ length: TOTAL_HABITACIONES }, (_, i) => i + 1);
  const ocupadas = rooms.filter(n => tenants.some(t => t.activo && matchesRoom(t.habitacion, n))).length;
  ...
  return (
    <>
      ...
      <div className="rg-rooms-grid">
        {rooms.map(n => {
          const tenant = tenants.find(t => t.activo && matchesRoom(t.habitacion, n));
          const paid = tenant && !!(tenant.pagos && tenant.pagos[selectedMonth]);

          if (!tenant) {
            return (
              <div className="rg-room-card rg-room-free" key={n}>
                <div className="rg-room-number">Hab. {n}</div>
                ...
                <button className="rg-btn" style={{ width: "100%", justifyContent: "center" }} onClick={() => onAddForRoom(n)}>
          ...
          return (
            <div className="rg-room-card rg-room-occupied" key={n}>
              <div className="rg-room-number">Hab. {n}</div>
