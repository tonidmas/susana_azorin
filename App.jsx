...
const GASTOS_FIJOS_KEYS = ["luz", "agua", "gas", "limpieza", "internet", "ibi", "comunidad", "reparaciones"];
const GASTOS_FIJOS_LABELS = { luz: "Luz", agua: "Agua", gas: "Gas", limpieza: "Limpieza", internet: "Internet", ibi: "IBI", comunidad: "Comunidad de Propietarios", reparaciones: "Reparaciones" };
...
export default function App() {
  ...
  function toggleActivo(t) {
    setTenants(prev => prev.map(p => (p.id === t.id ? { ...p, activo: !p.activo } : p)));
  }
        ...
        <div className="rg-main">
          ...
          {view === "inquilinos" && (
            <InquilinosView
              ...
              onToggleActivo={toggleActivo} onTogglePago={togglePagoQuick}
          ...
          {view === "historico" && (
            <HistoricoView tenants={tenants} onEdit={openEditTenant} onExport={handleExportHistorico} />
          )}
        </div>
      </div>
...
function InquilinosView({ tenants, selectedMonth, shiftMonth, monthLabel, onNew, onEdit, onDelete, onToggleActivo, onTogglePago, onImportClick, onExport }) {
  const sorted = [...tenants].sort((a, b) => String(a.habitacion).localeCompare(String(b.habitacion), "es", { numeric: true }));
    ...
    <>
      <div className="rg-topbar">
        <div>
          ...
          <div className="rg-sub">{tenants.length} registrado(s) · {tenants.filter(t => t.activo).length} activo(s)</div>
        </div>
      ...
      <div className="rg-card">
        {sorted.length === 0 ? (
          <div className="rg-empty">
            <Users size={30} />
            <div>Todavía no hay inquilinos registrados.</div>
            <div style={{ fontSize: 12, marginTop: 4 }}>Añade uno nuevo o importa tu Excel para empezar.</div>
          </div>
        ) : (
                ...
                {sorted.map(t => {
                  ...
                  return (
                    <tr key={t.id}>
                      ...
                      <td>
                        <button className="rg-icon-btn" onClick={() => onToggleActivo(t)} title={t.activo ? "Marcar como libre" : "Marcar como ocupada"}>
                          {t.activo ? (
                            <span className="rg-badge rg-badge-ok"><DoorClosed size={11} /> Ocupada</span>
                          ) : (
                            <span className="rg-badge rg-badge-neutral"><DoorOpen size={11} /> Libre</span>
                          )}
                        </button>
                      </td>
                      <td>
...
function HistoricoView({ tenants, onEdit, onExport }) {
        ...
        {historicos.length === 0 ? (
          <div className="rg-empty">
            ...
            <div style={{ fontSize: 12, marginTop: 4 }}>
              Cuando marques una habitación como "Libre" al terminar un contrato, ese inquilino aparecerá aquí.
            </div>
          </div>
        ) : (
          <div className="rg-table-wrap">
