...
function GastosView({ selectedMonth, monthLabel, shiftMonth, expenses, updateExpenseField, updateExpenseItems, incomeForMonth, fixedExpensesForMonth, managementFeeForMonth, totalExpensesForMonth, netProfitForMonth, yearData, selYear, onUploadFactura, onDeleteFactura, onViewDoc, updateGestionConfig }) {
  ...
  function ItemsBlock({ field, label, conceptos, addLabel }) {
    const items = monthExpenses[field] || [];
    const subtotal = items.reduce((s, it) => s + (Number(it.importe) || 0), 0);
    const factura = monthExpenses.facturas?.[field];
    return (
      <div className="rg-items-block">
          ...
          items.map(item => (
            <div className="rg-item-row" key={item.id}>
              <select
                ...
                onChange={(e) => updateItem(field, item.id, { concepto: e.target.value })}
              >
              ...
              <input
                ...
                value={item.importe}
                onChange={(e) => updateItem(field, item.id, { importe: e.target.value === "" ? "" : Number(e.target.value) })}
              />
              <button type="button" className="rg-icon-btn" onClick={() => removeItem(field, item.id)} title="Eliminar">
                <Trash2 size={13} />
              </button>
            </div>
          ))
        ...
        <div className="rg-items-actions">
          <button type="button" className="rg-btn-link" onClick={() => addItem(field, conceptos[0])}>
            <Plus size={12} /> {addLabel}
          </button>

          <input
            type="file" accept=".pdf,image/*"
            ref={(el) => { facturaRefs.current[field] = el; }}
            style={{ display: "none" }}
          ...
          {factura ? (
            ...
            <button type="button" className="rg-btn-link" onClick={() => facturaRefs.current[field]?.click()}>
  ...
  return (
    <>
      ...
      <div className="rg-card" style={{ padding: 20, marginBottom: 18 }}>
        ...
        <ItemsBlock field="reparacionesItems" label="Conceptos de reparación" conceptos={REPARACIONES_CONCEPTOS} addLabel="Añadir reparación" />

        <div className="rg-section-title">Otros gastos</div>
        <ItemsBlock field="otrosItems" label="Otros conceptos" conceptos={OTROS_CONCEPTOS} addLabel="Añadir otro gasto" />
