...
export default function App() {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        await window.storage.set(STORAGE_KEY, JSON.stringify({ tenants, expenses, irpfReduccion }), false);
      } catch (e) {
      ...
      <div className="rg-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rg-form-grid">
          ...
          <div className="rg-field">
            ...
            <input className="rg-input" type="number" min="0" step="0.01" value={form.renta} onChange={(e) => setRenta(Number(e.target.value))} />
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 22 }}>
          ...
          <button className="rg-btn" onClick={() => onSave(form)}>Guardar</button>
        </div>
      </div>
    </div>
  );
